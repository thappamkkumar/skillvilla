...
const FollowersPage = () => { 
	...
	const [followersList, setFollowersList] = useState([]);//state to store list of followers 
	...
	const apiCall = useCallback(async()=>{ 
      try 
        ...
        setLoading(true);
				...
				let url = `/follower-list?cursor=${cursor}`;
        ...
        const fetchedData = await serverConnection(url, reqiredData, authToken);
				...
				{
					setFollowersList(prevList => [...prevList, ...fetchedData.followers.data]);
					setCursor(fetchedData.followers.next_cursor);
					setHasMore(fetchedData.followers.next_cursor != null);
  ...
  useEffect(() => {
	...
	return ( 
		<>
			<PageSeo 
				title="Followers | SkillVilla"
				description="View the list of users who follow you on SkillVilla."
				keywords="followers, user followers, SkillVilla, professional connections"
			/>
						...
						<Navbar  className="nav_bar" >
							<Nav variant="pills"   className="w-100    " >
								...
								<Nav.Item className=" w-100" >
									<Nav.Link as={NavLink} to={`/user/${userId}/${ID}/followers` } className="  border border-1  rounded-0 text-center navigation_link ">
										Followers   
									</Nav.Link>
								</Nav.Item>
								<Nav.Item className=" w-100 " >
									<Nav.Link as={NavLink} to={`/user/${userId}/${ID}/followings`}   className="  border border-1  rounded-0 text-center navigation_link ">
										Followings   
									</Nav.Link>
								</Nav.Item>
							</Nav>	
						 ...
						 {followersList.length !== 0 && followersList.map((followersFollowing, index) => ( 
							 
							<FollowersFollowing  key={index} followersFollowing={followersFollowing} followType="followers" />
							 
						))}  
						
						
						{
							(!loading && followersList.length === 0) &&
							(
								<p className="text-center py-4 "><strong>
									No user in  follower  list.</strong>
								</p>
							)
						}
