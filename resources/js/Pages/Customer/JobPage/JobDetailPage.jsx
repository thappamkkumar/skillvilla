...
import JobActions from '../../../Components/Customer/CompanyJob/JobActions';
import JobCompanyAndUser from '../../../Components/Customer/CompanyJob/JobCompanyAndUser';
...
import JobApplicationCount from '../../../Components/Customer/CompanyJob/JobApplicationCount';
...
const JobDetailPage = () => { 
	...
	return ( 
		<>
			<PageSeo 
				title={jobDetail?.title ? `${jobDetail.title} | Job Detail | SkillVilla` : 'Job Detail | SkillVilla'}
				description={jobDetail?.title ? `Explore the job opportunity titled "${jobDetail.title}" on SkillVilla.` : 'Explore job listings from professionals on SkillVilla.'}
				keywords={jobDetail?.title ? `job detail, ${jobDetail.title}, SkillVilla, job opportunity` : 'job detail, SkillVilla, professional jobs'}
			/>
									...
									(
											<Row className=" w-100 m-0 ">
												<Col   xl={loggedUserData.id === jobDetail.user_id && jobTestQuestions.length > 0 ? 8 : 10} className="py-3  py-md-3 py-lg-4 px-2  px-md-3 px-lg-4  mx-auto rounded sub_main_container ">
													...
													<div   md={12} className="d-flex flex-wrap gap-2 align-items-center     justify-content-center p-0 py-3 m-0">
														{
															...
															?(
																...
																<JobApplicationCount 
																job_id={jobDetail.id} 
																totalApplications={jobDetail.applications_count} 
															...
															(		
																<JobActions 
																	job_id={jobDetail.id}
																	is_expired={jobDetail.is_expired}
																	already_applied={jobDetail.already_applied} 
																	attempts={jobDetail.attempts} 
																	has_saved={jobDetail.has_saved} 
																	applications={jobDetail.applications || []}
																	updateJobDetailSave={updateJobDetailSave}
																/>
															)
														}		
													</div>  
													...
													{
														loggedUserData.id != jobDetail.user_id &&
															<div   className="d-flex flex-wrap gap-2 align-items-center     justify-content-center py-2 w-100">
																		<JobActions 
																			job_id={jobDetail.id}
																			is_expired={jobDetail.is_expired}
																			already_applied={jobDetail.already_applied} 
