...
const SavedPostPage = () => { 
	...
	const apiCall = useCallback(async()=>{ 
		try
		{
			...
			let data = await serverConnection(`/get-saved-post?cursor=${postList.cursor}`, {userId: null, }, authToken);
	...
	return ( 
		<>
			<PageSeo 
				title="Saved Posts | SkillVilla"
				description="Access all the posts you've saved on SkillVilla."
				keywords="saved posts, bookmarks, SkillVilla, user saved content"
			/>
			...
			> 
			
				{
					 ...
					 (
							...
							<p className="no_posts_message post my-4">No saved posts yet. Save some interesting posts!</p>
							</div>
					 )
					 :
					 (
							 <>
								<div className="    px-2   px-sm-3 px-md-4 px-lg-5  py-2  ">
									<h3 className="fw-bold  ">Saved Posts</h3>
								 </div>
