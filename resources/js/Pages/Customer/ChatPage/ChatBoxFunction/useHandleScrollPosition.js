...
const useHandleScrollPosition = (firstRender, showScrollDownBTN, setShowScrollDownBTN, newMessages, messageList, scrollRef, chatId, dispatch ) => { 
...
/*	useEffect(()=>{
	if(scrollRef != null)
		{
			...
			dispatch(updateChatMessageState({type : 'SetScrollHeight', scrollHeight: initialPosition})); 
...
}, [chatId, dispatch, messageList.scrollHeightPosition]);
	...
	useEffect(()=>{
		...
		if(scrollRef != null)
		{
			...
			if(prevHeight < newHeight && scrollRef.current.scrollTop == 0 )
			{
				...
				dispatch(updateChatMessageState({type : 'SetScrollHeight', scrollHeight:newHeight}));
			}
  ...
  const handleScrollPosition = _.debounce(() => { 
    if (scrollRef.current) 
      ...
      dispatch(updateChatMessageState({type : 'SetScrollHeightPosition', scrollHeightPosition:scrollTop}));
