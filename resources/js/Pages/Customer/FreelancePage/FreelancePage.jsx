...
const FreelancePage = () => { 
	...
	const apiCall = useCallback(async()=>{ 
		try
		{
			setLoading(true);
			let userData = {}
			let url = `/get-interested-freelance-work?cursor=${freelanceList.cursor}`;
	...
	const handleScrollUpdate = useCallback((scrollTop) => {
		  dispatch(updateFreelanceState({type : 'setScrollHeightPosition', scrollHeightPosition: scrollTop}));
	...
	return ( 
		<>
			<PageSeo 
				title="Freelance Gigs | SkillVilla"
				description="Discover freelance gigs that match your interests or are posted by professionals you follow on SkillVilla."
				keywords="freelance gigs, SkillVilla, professional gigs, follow, interest-based work"
			/>
			...
			>
					<FreelanceHeader heading="Freelance Gigs" /> 
					{
						(freelanceList.freelanceList.length <= 0 && !loading) ?
							(
								<div className="    px-2   px-sm-3 px-md-4 px-lg-5  py-2  ">
									<p className="no_posts_message  ">
										You haven't followed any users yet, or they haven't posted any freelance work or task.
									</p>
								</div>
							):(
