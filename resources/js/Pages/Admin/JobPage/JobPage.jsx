...
const JobPage = () => { 
		...
		try
		{
		 ...
		 //console.log(data);
			...
			if(data != null && data.status == true )
			{
				...
				{ 
					dispatch(updateListState({type : 'SetData', listData: data.jobList.data})); 
				} 
				
				dispatch(updateListState({
					type : 'SetNextPageUrl',
					nextPageUrl: data.jobList.next_cursor != null ? `/admin/get-job-list?cursor=${data.jobList.next_cursor}` : null
					})); 
				dispatch(updateListState({
					type : 'SetPrevPageUrl',
					prevPageUrl: data.jobList.prev_cursor != null ? `/admin/get-job-list?cursor=${data.jobList.prev_cursor}` : null
					}));
	...
	return ( 
		<>
			<PageSeo 
				title="Manage Jobs | Admin | SkillVilla"
				description="View and manage all job postings submitted by users on SkillVilla."
				keywords="admin jobs, manage job postings, SkillVilla admin, job moderation"
			/>
