...
const PostPage = () => { 
		...
		try
		{
			...
			if(data != null && data.status == true )
			{
				if(data.postList.data.length != 0 )
				{
					...
					dispatch(updateListState({type : 'SetData', listData: data.postList.data})); 
				} 
				
				dispatch(updateListState({
					type : 'SetNextPageUrl',
					nextPageUrl: data.postList.next_cursor != null ? `/admin/get-post-list?cursor=${data.postList.next_cursor}` : null
					})); 
				dispatch(updateListState({
					type : 'SetPrevPageUrl',
					prevPageUrl: data.postList.prev_cursor != null ? `/admin/get-post-list?cursor=${data.postList.prev_cursor}` : null
					}));
	...
	return ( 
		<>
			<PageSeo 
				title="Manage Posts | Admin | SkillVilla"
				description="Browse and manage all user posts through the SkillVilla admin panel."
				keywords="admin posts, manage posts, SkillVilla admin, user content moderation"
			/>
