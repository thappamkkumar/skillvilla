...
const UpdateProfileImage = ({  setProfileData}) => {
	  ...
	  const uploadImage = useCallback(async (file) => {
        ...
        formData.append('profile_image', file); 
				...
				try { 
					...
					const data = await serverConnection('/admin/update-profile-image', formData, authToken, contentType ); 
			
			  
				 // console.log(data);
					if(data.status == true)
					{  
						const newImageUrl = `${data.image}?timestamp=${Date.now()}`;
						...
						setProfileData(pre=>{
								let updatedData = {...pre};
								if(updatedData.admin != null)
								{
									updatedData.admin.image = newImageUrl;
								} 
