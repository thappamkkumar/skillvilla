...
const ForgotNewPasswordVerification = ({ forgotPasswordData, setForgotPasswordData, setStep   }) => {
	...
	const handleStoringUser  = useCallback(async ( values, { setSubmitting }) => {
		...
		try
		{
			...
			{ 
				...
				setStored(true);
				setsubmitionMSG(response?.data?.message || 'Password has changed successfully.');
				
			}
			else
			{
				setStored(false);
				setsubmitionMSG(response?.data?.message || 'Failed to change password. Please try again later..');
			}
		}
		catch(e)
		{
...
export default  memo(ForgotNewPasswordVerification);
