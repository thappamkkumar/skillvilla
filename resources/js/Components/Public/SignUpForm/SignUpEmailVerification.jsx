...
const SignUpEmailVerification = ({ setStep, setSignUpData   }) => {
	...
	// validation rule for email and name
		const emailNameSchema = useMemo(() => Yup.object().shape({
			...
			name: Yup.string()
				...
				.test('not-only-spaces', 'Name cannot be just spaces', value => value && value.trim().length > 0),
		}), []);
...
export default  memo(SignUpEmailVerification);
