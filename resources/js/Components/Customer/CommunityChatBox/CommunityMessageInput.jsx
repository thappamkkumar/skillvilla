...
const CommunityMessageInput = ({ communityId, addNewMessageToChat }) => {
  ...
  const handleSendMessage = useCallback(async(event) => 
	{
			...
			try
			{
				...
				const resultData = await serverConnection('/community/upload-new-message', formData, authToken, contentType ); 
		...
		},[communityId, setSubmitting, authToken]
	);
...
export default memo(CommunityMessageInput);
